...
def generic_from_kspace_to_masked_kspace_and_mask(
    ...
    def from_kspace_to_masked_kspace_and_mask(images, kspaces):
        ...
        print("KSPACEEEEEEEEEEEEEEEEE", kspaces_scaled.shape)
        images_scaled = images * scale_factor
        kspaces_channeled = kspaces_scaled[..., None]
        print("KSPACEEEEEEEEEEEEEEEEE", kspaces_channeled.shape)
        images_channeled = images_scaled[..., None]
        if batch_size is not None:
            print("KSPACPPPPPPPPPPPPPPPPPPP32", kspaces_channeled[0].shape, mask[0].shape, images_channeled[0].shape)
            return (kspaces_channeled[0], mask[0]), images_channeled[0]
        else:
            print("KSPACNNNNNNNNNNNNNNNNNNN", kspaces_channeled.shape, mask.shape, images_channeled.shape)
...
def generic_prepare_mask_and_kspace(scale_factor=1):
    def prepare(mask, kspaces):
        ...
        # print("SHHHHHHHHHHHAPPPPPPPPEEEEEEEEEEEEEE")
        # print(shape)
        num_cols = shape[-1]
        ...
        # print("MAASSSSSSSSKKKKKKKKKK")
        # print(final_mask_reshaped)
